...
bool SocketListener::start(CallObj& co)
{
  ...
  // This thread will be running and when client calls connect, Here we create new thread and socket
  // and then call the ref(co) , here we call the functor of the client handler for each client. 
  // The client handler functor will directly communicate with the client and sending back the data
  // 
